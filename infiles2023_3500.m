...
tdumpfolder='C:\HYSPLIT\working_appalair\Summer2023\allsummer_NAM\3500';
InfilePath = 'C:\HYSPLIT\working_appalair\Summer2023\INFILES\3500';
...
fmt = '%s\n';
fileid=fopen(fullfile(InfilePath,'INFILE_3500_LO.txt'),'w');
for i=1:iLO
    fprintf(fileid,fmt,LOfiles(i,:));
end
fclose(fileid);
fileid=fopen(fullfile(InfilePath,'INFILE_3500_AV.txt'),'w');
for i=1:iAV
    fprintf(fileid,fmt,AVfiles(i,:));
end
fclose(fileid);
fileid=fopen(fullfile(InfilePath,'INFILE_3500_HI.txt'),'w');
for i=1:iHI
    fprintf(fileid,fmt,HIfiles(i,:));
end
fclose(fileid);
fileid=fopen(fullfile(InfilePath,'INFILE_3500_VH.txt'),'w');
for i=1:iVH
