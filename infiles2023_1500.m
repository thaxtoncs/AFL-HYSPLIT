...
tdumpfolder='C:\HYSPLIT\working_appalair\Summer2023\allsummer_NAM\1500';
InfilePath = 'C:\HYSPLIT\working_appalair\Summer2023\INFILES\1500';
...
fmt = '%s\n';
fileid=fopen(fullfile(InfilePath,'INFILE_1500_LO.txt'),'w');
for i=1:iLO
    fprintf(fileid,fmt,LOfiles(i,:));
end
fclose(fileid);
fileid=fopen(fullfile(InfilePath,'INFILE_1500_AV.txt'),'w');
for i=1:iAV
    fprintf(fileid,fmt,AVfiles(i,:));
end
fclose(fileid);
fileid=fopen(fullfile(InfilePath,'INFILE_1500_HI.txt'),'w');
for i=1:iHI
    fprintf(fileid,fmt,HIfiles(i,:));
end
fclose(fileid);
fileid=fopen(fullfile(InfilePath,'INFILE_1500_VH.txt'),'w');
for i=1:iVH
